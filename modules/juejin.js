...
const tableName = config.db_prefix + 'juejin'
let date = moment().format('YYYY-MM-DD')
const sortId = 45;
let params = {"id_type":2,"sort_type":200,"cate_id":"6809637767543259144","tag_id":"6809640407484334093","cursor":"0","limit":20};

function getInfo () {
  // 利用superagent 模块发送请求，注意请求头的设置和POST请求体数据（请求参数）
  log.info('掘金前端热门数据获取完毕!')
  superagent.post('https://api.juejin.cn/recommend_api/v1/article/recommend_cate_tag_feed?aid=2608&uuid=7017632423543490052').send(params).set('X-Agent', 'Juejin/Web').end((err, res) => {
    if (err) {
